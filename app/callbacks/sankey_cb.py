...
def register(app):
    ...
    def update_sankey(selected_repos, month_range, include_bots, theme):
        ...
        df_latency = load_pr_latency(selected_repos, start_month=start_month, end_month=end_month)
        df_latency = apply_millers_law(df_latency, selected_repos)
        
        label = ["Opened PRs", "Reviewed", "Unreviewed", "Merged", "Closed Without Merge", "Still Open"]
        ...
        if not df_latency.empty:
            ...
            if len(selected_repos) > 1:
                box_fig = px.box(df_latency, x='repo', y='latency_hours', color='repo')
            else:
